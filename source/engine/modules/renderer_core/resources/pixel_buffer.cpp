...
namespace ray::renderer_core_api::resources
{
    ...
    void PixelBuffer::ExportToFile(const std::wstring& path)
    {
        // TODO: 
    }

    void PixelBuffer::CreateTextureResource(ID3D12Device* device, const D3D12_RESOURCE_DESC& desc, D3D12_CLEAR_VALUE clearValue)
    {
        Destroy();
    ...
    D3D12_RESOURCE_DESC PixelBuffer::DescribeTex2D(u32 width, u32 height, u32 arraySize, u32 numMips, DXGI_FORMAT format, u32 flags)
    {
        _width = width;
        _height = height;
        ...
        desc.Alignment = 0;
        desc.DepthOrArraySize = static_cast<u16>(arraySize);
        desc.Dimension = D3D12_RESOURCE_DIMENSION_TEXTURE2D;
        desc.Flags = static_cast<D3D12_RESOURCE_FLAGS>(flags);
        desc.Format = GetBaseFormat(format);
        desc.Height = static_cast<u32>(height);
        desc.Layout = D3D12_TEXTURE_LAYOUT_UNKNOWN;
        desc.MipLevels = static_cast<u16>(numMips);
        desc.SampleDesc.Count = 1;
        desc.SampleDesc.Quality = 0;
        desc.Width = static_cast<u64>(width);
        return desc;
    }
