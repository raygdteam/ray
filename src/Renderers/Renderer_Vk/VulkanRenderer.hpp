...
namespace ray::renderer::vulkan
{
	class VulkanRenderer : public ray::renderer::IRenderer
	{
		...
		classes::texture::DepthBuffer _depthBuffer;
		
		uint32_t _imageIndex = 0;
		uint32_t _graphicsFamilyIndex, _presentFamilyIndex;
		vk::Extent2D _screenResolution;
		const u8 MAX_FRAMES_IN_FLIGHT = 2;
		u8 _currentFrame = 0; // guaranteed to be at least 1

		std::vector<vk::UniqueSemaphore> _acquireSemaphores, _releaseSemaphores;
