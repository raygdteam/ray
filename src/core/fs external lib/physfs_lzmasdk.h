...
static void LookToRead_Init(CLookToRead* p) { p->pos = p->size = 0; }
...
static MY_NO_INLINE SRes ReadUi32s(CSzData* sd2, UInt32 numItems, CSzBitUi32s* crcs, ISzAlloc* alloc)
{
	...
	for (i = 0; i < numItems; i++)
		if (SzBitArray_Check(defs, i)) { SZ_READ_32(vals[i]); }
		else
			vals[i] = 0;
	*sd2 = sd;
	return SZ_OK;
							...
							ISzAlloc* alloc)
{
	...
	{
		UInt32 i;
		for (i = 0; i < numCodersOutStreams; i++) { RINOK(ReadNumber(&sd, p->CoderUnpackSizes + i)); }
	}
...
)
{
...

	{
		...
		for (;;)
		{
			...
			if (type >= ((UInt32)1 << 8)) { SKIP_DATA(sd, size); }
			else
				...
				case k7zIdName:
					{
						...
						if (external == 0) { SKIP_DATA(sd, namesSize); }
						break;
					}
			...
			for (i = 0; i < numFiles; i++, mask >>= 1)
			{
				...
				if (remSubStreams == 0)
				{
					for (;;)
					{
						...
						numSubStreams = 1;
						if (ssi.sdNumSubStreams.Data) { RINOK(SzReadNumber32(&ssi.sdNumSubStreams, &numSubStreams)); }
	...
	if (res == SZ_OK)
	{
		...
		if (CrcCalc(buf.data, nextHeaderSizeT) == nextHeaderCRC)
		{
			...
			if (res == SZ_OK && type == k7zIdEncodedHeader)
			{
				...
				SzAr_Init(&tempAr);
				res = SzReadAndDecodePackedStreams(inStream, &sd, &tempBuf, 1, p->startPosAfterHeader, &tempAr,
													allocTemp);
				SzAr_Free(&tempAr, allocTemp);

				if (res != SZ_OK) { Buf_Free(&tempBuf, allocTemp); }
				else
				{
							...
							Byte* tempBuf[])
{
	...
	for (ci = 0; ci < folder->NumCoders; ci++)
	{
		...
		if (IS_MAIN_METHOD((UInt32)coder->MethodID))
		{
			...
			{
				RINOK(SzDecodeLzma(propsData + coder->PropsOffset, coder->PropsSize, inSize, inStream, outBufCur,
...
static void Delta_Decode(Byte* state, unsigned delta, Byte* data, SizeT size)
{
	...
	MyMemCpy(buf, state, delta);
	{
		...
		for (i = 0; i < size;)
		{
			for (j = 0; j < delta && i < size; i++, j++) { buf[j] = data[i] = (Byte)(buf[j] + data[i]); }
		}
	}
	if (j == delta)
		j = 0;
