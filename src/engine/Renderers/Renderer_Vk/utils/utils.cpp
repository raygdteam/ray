...
vk::UniqueFence create_fence(vk::UniqueDevice const& device)
{
...
vk::UniqueRenderPass create_render_pass(vk::PhysicalDevice const& physicalDevice, vk::SurfaceKHR const& surface, vk::UniqueDevice const& device, vk::Format depthFormat)
{
	...
	*/
	...
	//vk::SubpassDependency dependency(VK_SUBPASS_EXTERNAL, 0, vk::PipelineStageFlagBits::eEarlyFragmentTests, vk::PipelineStageFlagBits::eEarlyFragmentTests, vk::AccessFlags(), vk::AccessFlagBits::eDepthStencilAttachmentRead | vk::AccessFlagBits::eDepthStencilAttachmentWrite);

	return device->createRenderPassUnique(vk::RenderPassCreateInfo(vk::RenderPassCreateFlags(), 2, attachmentDescriptions, 1, &subpass, 1, &dependency));
}
}
