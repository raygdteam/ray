						...
						const PHYSFS_sint64 ts, void* arc)
{
	...
	for (i = 0; i < count; i++)
	{
		...
		if (!(type & VDF_ENTRY_DIR)) { BAIL_IF_ERRPASS(!UNPK_addEntry(arc, name, 0, ts, ts, jump, size), 0); }
	} /* for */
							...
							int forWriting, int* claimed)
{
	...
	if ((memcmp(sig, VDF_SIGNATURE_G1, VDF_SIGNATURE_LENGTH) != 0) &&
		(memcmp(sig, VDF_SIGNATURE_G2, VDF_SIGNATURE_LENGTH) != 0)) { BAIL(PHYSFS_ERR_UNSUPPORTED, NULL); } /* if */
