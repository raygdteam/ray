...
const uint32_t actual_fiber_count = std::max<uint32_t>(actual_thread_count * fiber_per_thread_count, 12);
...
FiberHandle* original_thread_fibers(new FiberHandle[actual_thread_count]);
FiberSlot* fibers(new FiberSlot[actual_fiber_count]);
...
void TaskSystem::init()
{
	...
	// log("TaskSystem: initialized with %u fibers and %u threads", actual_fiber_count, actual_thread_count);
}
