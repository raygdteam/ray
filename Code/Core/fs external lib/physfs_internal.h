...
void* __PHYSFS_initSmallAlloc(void* ptr, size_t len);
 ...
 */
PHYSFS_Io* __PHYSFS_createNativeIo(const char* path, int mode);
 ...
 */
int __PHYSFS_readAll(PHYSFS_Io* io, void* buf, size_t len);
...
void* UNPK_addEntry(void* opaque, char* name, int isdir,
					PHYSFS_sint64 ctime, PHYSFS_sint64 mtime,
 ...
 */
int __PHYSFS_platformStat(const char* fn, PHYSFS_Stat* stat, int follow);
