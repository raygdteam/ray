...
static inline PHYSFS_ErrorCode errcodeFromWinApi(void) { return errcodeFromWinApiError(GetLastError()); }
/* errcodeFromWinApi */
										...
										WPARAM wp, LPARAM lparam)
{
	...
	if ((msg != WM_DEVICECHANGE) ||
		...
		((lpdbv->dbcv_flags & DBTF_MEDIA) == 0)) { return DefWindowProcW(hwnd, msg, wp, lparam); }
	/* else if */
...
void __PHYSFS_platformDetectAvailableCDs(PHYSFS_StringCallback cb, void* data) { detectAvailableCDs(cb, data); }
...
void __PHYSFS_platformDeinit(void) { deinitCDThread(); } /* __PHYSFS_platformDeinit */


void* __PHYSFS_platformGetThreadID(void) { return ((void *)((size_t)GetCurrentThreadId())); }
...
PHYSFS_sint64 __PHYSFS_platformRead(void* opaque, void* buf, PHYSFS_uint64 len)
{
	...
	if (ReadFile(h, buf, len - 1, &totalRead, NULL) != TRUE)
		BAIL(PHYSFS_ERR_INVALID_ARGUMENT, -1);
...
void __PHYSFS_platformReleaseMutex(void* mutex) { LeaveCriticalSection((LPCRITICAL_SECTION)mutex); }
