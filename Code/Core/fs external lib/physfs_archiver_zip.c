 ...
 * Bridge physfs allocation functions to zlib's format...
 */
static void zlibPhysfsFree(voidpf opaque, voidpf address) { ((PHYSFS_Allocator *)opaque)->Free(address); }
/* zlibPhysfsFree */
...
static PHYSFS_sint64 ZIP_tell(PHYSFS_Io* io) { return ((ZIPfileinfo *)io->opaque)->uncompressed_position; }
/* ZIP_tell */
							...
							int forWriting, int* claimed)
{
	...
	*claimed = 1;
	...
	if (!__PHYSFS_DirTreeInit(&info->tree, sizeof(ZIPentry)))
		goto ZIP_openarchive_failed;
...
static PHYSFS_Io* ZIP_openRead(void* opaque, const char* filename)
{
	...
	else
	{
		...
		if (io->read(io, crypto_header, 12) != 12)
			goto ZIP_openRead_failed;
		if (!zip_prep_crypto_keys(finfo, crypto_header, password))
			goto ZIP_openRead_failed;
	} /* if */
...
static int ZIP_stat(void* opaque, const char* filename, PHYSFS_Stat* stat)
{
	...
	if (!zip_resolve(info->io, info, entry))
		return 0;
	if (entry->resolved == ZIP_DIRECTORY)
	{
		stat->filesize = 0;
