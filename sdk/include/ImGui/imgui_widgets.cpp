...
static ImVec2 InputTextCalcTextSizeW(const ImWchar* text_begin, const ImWchar* text_end,
									const ImWchar** remaining = nullptr, ImVec2* out_offset = nullptr,
	...
	if (g.DragDropActive && (flags & ImGuiButtonFlags_PressedOnDragDropHold) && !(g.DragDropSourceFlags &
		...
		if (IsItemHovered(ImGuiHoveredFlags_AllowWhenBlockedByActiveItem))
		{
			...
			if (CalcTypematicPressedRepeatAmount(g.HoveredIdTimer + 0.0001f,
												g.HoveredIdTimer + 0.0001f - g.IO.DeltaTime, 0.01f, 0.70f))
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, nullptr, &label_size,
				...
				const ImVec4& tint_col, const ImVec4& border_col)
{
	...
	else { window->DrawList->AddImage(user_texture_id, bb.Min, bb.Max, uv0, uv1, GetColorU32(tint_col)); }
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	IMGUI_TEST_ENGINE_ITEM_INFO(id, label, window->DC.ItemFlags | ImGuiItemStatusFlags_Checkable | (*v ?
	...
	if (overlay_size.x > 0.0f)
		RenderTextClipped(
			ImVec2(ImClamp(fill_br.x + style.ItemSpacing.x, bb.Min.x,
							bb.Max.x - overlay_size.x - style.ItemInnerSpacing.x), bb.Min.y), bb.Max, overlay, nullptr,
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
 ...
 ImGuiComboFlags_NoPreview)); // Can't use both flags together
	...
	if (preview_value != nullptr && !(flags & ImGuiComboFlags_NoPreview))
		RenderTextClipped(frame_bb.Min + style.FramePadding, ImVec2(value_x2, frame_bb.Max.y), preview_value, nullptr,
						nullptr, ImVec2(0.0f, 0.0f));
	if (label_size.x > 0)
						...
						const char* format, float power, ImGuiDragFlags flags)
{
	...
	bool is_already_past_limits_and_pushing_outward = has_min_max && ((*v >= v_max && adjust_delta > 0.0f) || (*v <=
	...
	else { v_cur += (TYPE)g.DragCurrentAccum; }
	...
	else { g.DragCurrentAccum -= (float)((SIGNEDTYPE)v_cur - (SIGNEDTYPE)*v); }
	...
	case ImGuiDataType_S16:
		{
			...
			bool r = DragBehaviorT<ImS32, ImS32, float>(ImGuiDataType_S32, &v32, v_speed,
														...
														v_max ? *(const ImS16*)v_max : IM_S16_MAX, format, power,
														flags);
	...
	case ImGuiDataType_U16:
		{
			...
			bool r = DragBehaviorT<ImU32, ImS32, float>(ImGuiDataType_U32, &v32, v_speed,
														...
														v_max ? *(const ImU16*)v_max : IM_U16_MAX, format, power,
														flags);
			if (r) *(ImU16*)v = (ImU16)v32;
			return r;
		}
	case ImGuiDataType_S32: return DragBehaviorT<ImS32, ImS32, float>(data_type, (ImS32*)v, v_speed,
																	v_min ? *(const ImS32*)v_min : IM_S32_MIN,
																	v_max ? *(const ImS32*)v_max : IM_S32_MAX, format,
																	power, flags);
	case ImGuiDataType_U32: return DragBehaviorT<ImU32, ImS32, float>(data_type, (ImU32*)v, v_speed,
																	v_min ? *(const ImU32*)v_min : IM_U32_MIN,
																	v_max ? *(const ImU32*)v_max : IM_U32_MAX, format,
																	power, flags);
	case ImGuiDataType_S64: return DragBehaviorT<ImS64, ImS64, double>(data_type, (ImS64*)v, v_speed,
																		v_min ? *(const ImS64*)v_min : IM_S64_MIN,
																		v_max ? *(const ImS64*)v_max : IM_S64_MAX,
																		format, power, flags);
	case ImGuiDataType_U64: return DragBehaviorT<ImU64, ImS64, double>(data_type, (ImU64*)v, v_speed,
																		v_min ? *(const ImU64*)v_min : IM_U64_MIN,
																		v_max ? *(const ImU64*)v_max : IM_U64_MAX,
																		format, power, flags);
	case ImGuiDataType_Float: return DragBehaviorT<float, float, float>(data_type, (float*)v, v_speed,
																		v_min ? *(const float*)v_min : -FLT_MAX,
																		v_max ? *(const float*)v_max : FLT_MAX, format,
																		power, flags);
						...
						const void* v_max, const char* format, float power)
{
	...
	char value_buf[64];
	const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, v,
																format);
							...
							ImRect* out_grab_bb)
{
	...
	if (g.ActiveId == id)
	{
		...
		if (set_new_value)
		{
			...
			{
				// Linear slider
				if (is_decimal) { v_new = ImLerp(v_min, v_max, clicked_t); }
				else
				{
								...
								void* data_ptr, const char* format)
{
	...
	bool value_changed = InputTextEx(label, nullptr, data_buf, IM_ARRAYSIZE(data_buf), bb.GetSize(), flags);
	if (init)
	{
	...
	if (value_changed)
	{
		value_changed = DataTypeApplyOpFromText(data_buf, g.InputTextState.InitialTextA.Data, data_type, data_ptr,
												nullptr);
		if (value_changed)
			MarkItemEdited(id);
	}
						...
						const void* step_fast, const char* format, ImGuiInputTextFlags flags)
{
	...
	if (step != nullptr)
	{
		...
		if (InputText("", buf, IM_ARRAYSIZE(buf), flags))
			...
			value_changed = DataTypeApplyOpFromText(buf, g.InputTextState.InitialTextA.Data, data_type, data_ptr,
													format);
						...
						ImGuiInputTextFlags flags)
{
	...
	return InputScalar(label, ImGuiDataType_Float, (void*)v, (void*)(step > 0.0f ? &step : nullptr),
...
bool ImGui::InputInt(const char* label, int* v, int step, int step_fast, ImGuiInputTextFlags flags)
{
	...
	return InputScalar(label, ImGuiDataType_S32, (void*)v, (void*)(step > 0 ? &step : nullptr),
...
{
	flags |= ImGuiInputTextFlags_CharsScientific;
	return InputScalar(label, ImGuiDataType_Double, (void*)v, (void*)(step > 0.0 ? &step : nullptr),
...
static void STB_TEXTEDIT_LAYOUTROW(StbTexteditRow* r, STB_TEXTEDIT_STRING* obj, int line_start_idx)
{
	...
	const ImVec2 size = InputTextCalcTextSizeW(text + line_start_idx, text + obj->CurLenW, &text_remaining, nullptr,
												true);
	r->x0 = 0.0f;
	r->x1 = size.x;
						...
						ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
	...
	const bool user_scroll_finish = is_multiline && state != nullptr && g.ActiveId == 0 && g.ActiveIdPreviousFrame ==
		GetScrollbarID(draw_window, ImGuiAxis_Y);
	const bool user_scroll_active = is_multiline && state != nullptr && g.ActiveId == GetScrollbarID(
	...
	if (is_password && !is_displaying_hint)
	{
		...
		IM_ASSERT(password_font->Glyphs.empty() && password_font->IndexAdvanceX.empty() && password_font->IndexLookup.
empty());
	...
	if (render_cursor || render_selection)
	{
		...
		if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
		{
			...
			draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll, col, buf_display,
											buf_display_end, 0.0f, is_multiline ? nullptr : &clip_rect);
		}
	...
	else
	{
		...
		if (is_multiline)
			text_size = ImVec2(size.x, InputTextCalcTextLenAndLineCount(buf_display, &buf_display_end) * g.FontSize);
			// We don't need width
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
		...
		ImGuiColorEditFlags_NoInputs) == 0)
	{
		...
		const float w_item_last = ImMax(
			1.0f, (float)(int)(w_inputs - (w_item_one + style.ItemInnerSpacing.x) * (components - 1)));
		...
		for (int n = 0; n < components; n++)
		{
			...
			else { value_changed |= DragInt(ids[n], &i[n], 1.0f, 0, hdr ? 0 : 255, fmt_table_int[fmt_idx][n]); }
			if (!(flags & ImGuiColorEditFlags_NoOptions))
												...
												ImVec2 grid_off, float rounding, int rounding_corners_flags)
{
	...
	else { window->DrawList->AddRectFilled(p_min, p_max, col, rounding, rounding_corners_flags); }
}
...
bool ImGui::TreeNodeBehaviorIsOpen(ImGuiID id, ImGuiTreeNodeFlags flags)
{
	...
	if (g.NextItemData.Flags & ImGuiNextItemDataFlags_HasOpen)
	{
		...
		else
		{
			...
			else { is_open = stored_value != 0; }
		}
	}
	else { is_open = storage->GetInt(id, (flags & ImGuiTreeNodeFlags_DefaultOpen) ? 1 : 0) != 0; }
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
	...
	const float text_base_offset_y = ImMax(padding.y, window->DC.CurrLineTextBaseOffset);
	...
	if (!item_add)
	{
		...
		IMGUI_TEST_ENGINE_ITEM_INFO(window->DC.LastItemId, label, window->DC.ItemFlags | (is_leaf ? 0 :
 ImGuiItemStatusFlags_Openable) | (is_open ? ImGuiItemStatusFlags_Opened : 0));
		return is_open;
	}
	...
	if (display_frame)
	{
		...
		else { RenderTextClipped(text_pos, frame_bb.Max, label, label_end, &label_size); }
	}
	else
	{
		...
		if (flags & ImGuiTreeNodeFlags_Bullet)
			RenderBullet(window->DrawList,
						frame_bb.Min + ImVec2(text_offset_x * 0.5f, g.FontSize * 0.50f + text_base_offset_y), text_col);
		else if (!is_leaf)
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
		...
		// (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
	{
		...
		ImGuiWindow* child_menu_window = (g.BeginPopupStack.Size < g.OpenPopupStack.Size && g.OpenPopupStack[g
											...
											: nullptr;
		if (g.HoveredWindow == window && child_menu_window != nullptr && !(window->Flags & ImGuiWindowFlags_MenuBar))
		{
	...
	IMGUI_TEST_ENGINE_ITEM_INFO(id, label, window->DC.ItemFlags | ImGuiItemStatusFlags_Openable | (menu_is_open ?
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
	...
	for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
	{
		...
		if (most_recently_selected_tab == nullptr || most_recently_selected_tab->LastFrameSelected < tab->
			LastFrameSelected)
	...
	else { tab_bar->ScrollingSpeed = 0.0f; }
										...
										ImGuiID close_button_id)
{
	...
	if (flags & ImGuiTabItemFlags_UnsavedDocument)
	{
		...
		RenderTextClippedEx(draw_list, unsaved_marker_pos, bb.Max - frame_padding, TAB_UNSAVED_MARKER, nullptr,
							nullptr);
	}
