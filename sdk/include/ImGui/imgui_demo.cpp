...
void ImGui::ShowDemoWindow(bool* p_open)
{
	...
	if (BeginMenuBar())
	{
		...
		if (BeginMenu("Examples"))
		{
			...
			MenuItem("Simple overlay", nullptr, &show_app_simple_overlay);
			MenuItem("Manipulating window titles", nullptr, &show_app_window_titles);
			MenuItem("Custom rendering", nullptr, &show_app_custom_rendering);
			MenuItem("Documents", nullptr, &show_app_documents);
			EndMenu();
		}
		if (BeginMenu("Help"))
		{
	...
	if (CollapsingHeader("Configuration"))
	{
		...
		{
			CheckboxFlags("io.ConfigFlags: NavEnableKeyboard", (unsigned int *)&io.ConfigFlags,
						ImGuiConfigFlags_NavEnableKeyboard);
			CheckboxFlags("io.ConfigFlags: NavEnableGamepad", (unsigned int *)&io.ConfigFlags,
			...
			CheckboxFlags("io.ConfigFlags: NavEnableSetMousePos", (unsigned int *)&io.ConfigFlags,
			...
			CheckboxFlags("io.ConfigFlags: NoMouseCursorChange", (unsigned int *)&io.ConfigFlags,
		...
		if (TreeNode("Backend Flags"))
		{
			...
			CheckboxFlags("io.BackendFlags: HasMouseCursors", (unsigned int *)&backend_flags,
						ImGuiBackendFlags_HasMouseCursors);
			CheckboxFlags("io.BackendFlags: HasSetMousePos", (unsigned int *)&backend_flags,
						ImGuiBackendFlags_HasSetMousePos);
			CheckboxFlags("io.BackendFlags: RendererHasVtxOffset", (unsigned int *)&backend_flags,
...
static void ShowDemoWindowWidgets()
{
	...
	if (ImGui::TreeNode("Data Types"))
	{
		...
		ImGui::DragScalar("drag s8", ImGuiDataType_S8, &s8_v, drag_speed, drag_clamp ? &s8_zero : nullptr,
						drag_clamp ? &s8_fifty : nullptr);
		ImGui::DragScalar("drag u8", ImGuiDataType_U8, &u8_v, drag_speed, drag_clamp ? &u8_zero : nullptr,
						drag_clamp ? &u8_fifty : nullptr, "%u ms");
		ImGui::DragScalar("drag s16", ImGuiDataType_S16, &s16_v, drag_speed, drag_clamp ? &s16_zero : nullptr,
						drag_clamp ? &s16_fifty : nullptr);
		ImGui::DragScalar("drag u16", ImGuiDataType_U16, &u16_v, drag_speed, drag_clamp ? &u16_zero : nullptr,
						drag_clamp ? &u16_fifty : nullptr, "%u ms");
		ImGui::DragScalar("drag s32", ImGuiDataType_S32, &s32_v, drag_speed, drag_clamp ? &s32_zero : nullptr,
						drag_clamp ? &s32_fifty : nullptr);
		ImGui::DragScalar("drag u32", ImGuiDataType_U32, &u32_v, drag_speed, drag_clamp ? &u32_zero : nullptr,
						drag_clamp ? &u32_fifty : nullptr, "%u ms");
		ImGui::DragScalar("drag s64", ImGuiDataType_S64, &s64_v, drag_speed, drag_clamp ? &s64_zero : nullptr,
						drag_clamp ? &s64_fifty : nullptr);
		ImGui::DragScalar("drag u64", ImGuiDataType_U64, &u64_v, drag_speed, drag_clamp ? &u64_zero : nullptr,
		...
		ImGui::InputScalar("input s32 hex", ImGuiDataType_S32, &s32_v, inputs_step ? &s32_one : nullptr, nullptr,
		...
		ImGui::InputScalar("input u32 hex", ImGuiDataType_U32, &u32_v, inputs_step ? &u32_one : nullptr, nullptr,
							"%08X", ImGuiInputTextFlags_CharsHexadecimal);
	...
	if (ImGui::TreeNode("Querying Status (Active/Focused/Hovered etc.)"))
	{
		...
		static char str[16] = {};
		ImGui::Combo("Item Type", &item_type,
					"Text\0Button\0Button (w/ repeat)\0Checkbox\0SliderFloat\0InputText\0InputFloat\0InputFloat3\0ColorEdit4\0MenuItem\0TreeNode (w/ double-click)\0ListBox\0");
		ImGui::SameLine();
...
static void ShowDemoWindowLayout()
{
	...
	if (ImGui::TreeNode("Child windows"))
	{
		...
		{
			ImGuiWindowFlags window_flags = ImGuiWindowFlags_HorizontalScrollbar | (disable_mouse_wheel
	...
	if (ImGui::TreeNode("Scrolling"))
	{
		...
		ImGui::SameLine(140);
		scroll_to_pos |= ImGui::DragFloat("##pos", &scroll_to_pos_px, 1.00f, 0, 9999, "X/Y = %.0f px");
...
void ImGui::ShowAboutWindow(bool* p_open)
{
	...
	if (show_config_info)
	{
		...
		Separator();
		Text("sizeof(size_t): %d, sizeof(ImDrawIdx): %d, sizeof(ImDrawVert): %d", (int)sizeof(size_t),
		...
		Separator();
		Text("io.Fonts: %d fonts, Flags: 0x%08X, TexSize: %d,%d", io.Fonts->Fonts.Size, io.Fonts->Flags,
...
bool ImGui::ShowStyleSelector(const char* label)
{
	...
	if (Combo(label, &style_idx, "Classic\0Dark\0Light\0"))
	{
		switch (style_idx)
		{
		case 0: StyleColorsClassic();
			break;
		case 1: StyleColorsDark();
			break;
		case 2: StyleColorsLight();
			break;
		}
		return true;
	}
...
void ImGui::ShowStyleEditor(ImGuiStyle* ref)
{
		...
		style.GrabRounding = style.FrameRounding; // Make GrabRounding always the same value as FrameRounding
	{
		...
		if (Checkbox("WindowBorder", &window_border)) style.WindowBorderSize = window_border ? 1.0f : 0.0f;
	}
	SameLine();
	{
		bool frame_border = (style.FrameBorderSize > 0.0f);
		if (Checkbox("FrameBorder", &frame_border)) style.FrameBorderSize = frame_border ? 1.0f : 0.0f;
	}
	SameLine();
	{
		bool popup_border = (style.PopupBorderSize > 0.0f);
		if (Checkbox("PopupBorder", &popup_border)) style.PopupBorderSize = popup_border ? 1.0f : 0.0f;
	}
	...
	if (BeginTabBar("##tabs", ImGuiTabBarFlags_None))
	{
		...
		if (BeginTabItem("Colors"))
		{
			...
			RadioButton("Opaque", &alpha_flags, 0);
			SameLine();
			RadioButton("Alpha", &alpha_flags, ImGuiColorEditFlags_AlphaPreview);
			SameLine();
			RadioButton("Both", &alpha_flags, ImGuiColorEditFlags_AlphaPreviewHalf);
			SameLine();
			HelpMarker(
			...
			for (int i = 0; i < ImGuiCol_COUNT; i++)
			{
				...
				if (memcmp(&style.Colors[i], &ref->Colors[i], sizeof(ImVec4)) != 0)
				{
					...
					SameLine(0.0f, style.ItemInnerSpacing.x);
					if (Button("Revert")) style.Colors[i] = ref->Colors[i];
				}
				SameLine(0.0f, style.ItemInnerSpacing.x);
		...
		if (BeginTabItem("Fonts"))
		{
			...
			for (int i = 0; i < atlas->Fonts.Size; i++)
			{
				...
				if (font_details_opened)
				{
					...
					Text("Texture surface: %d pixels (approx) ~ %dx%d", font->MetricsTotalSurface, (int)surface_sqrt,
						(int)surface_sqrt);
					for (int config_i = 0; config_i < font->ConfigDataCount; config_i++)
						if (const ImFontConfig* cfg = &font->ConfigData[config_i])
							BulletText("Input %d: \'%s\', Oversample: (%d,%d), PixelSnapH: %d", config_i, cfg->Name,
					...
					if (TreeNode("Glyphs", "Glyphs (%d)", font->Glyphs.Size))
					{
						...
						for (int base = 0; base < 0x10000; base += 256)
						{
							...
							if (count > 0 && TreeNode((void*)(intptr_t)base, "U+%04X..U+%04X (%d %s)", base, base + 255,
													count, count > 1 ? "glyphs" : "glyph"))
							{
								...
								for (int n = 0; n < 256; n++)
								{
									...
									if (glyph)
										font->RenderChar(draw_list, cell_size, cell_p1, GetColorU32(ImGuiCol_Text),
														(ImWchar)(base + n));
			...
			if (TreeNode("Atlas texture", "Atlas texture (%dx%d pixels)", atlas->TexWidth, atlas->TexHeight))
			{
				...
				Image(atlas->TexID, ImVec2((float)atlas->TexWidth, (float)atlas->TexHeight), ImVec2(0, 0), ImVec2(1, 1),
					tint_col, border_col);
				TreePop();
			}
			...
			if (DragFloat("window scale", &window_scale, 0.005f, 0.3f, 2.0f, "%.2f")) // scale only this window
...
static void ShowExampleAppLog(bool* p_open)
{
	...
	if (ImGui::SmallButton("[Debug] Add 5 entries"))
	{
		...
		for (int n = 0; n < 5; n++)
		{
			...
			const char* words[] = {
				"Bumfuzzled", "Cattywampus", "Snickersnee", "Abibliophobia", "Absquatulate", "Nincompoop",
				"Pauciloquent"
			};
			log.AddLog("[%05d] [%s] Hello, current time is %.1f, here's a word: '%s'\n",
						ImGui::GetFrameCount(), categories[counter % IM_ARRAYSIZE(categories)], ImGui::GetTime(),
