...
void ImGuiTextBuffer::appendfv(const char* fmt, va_list args)
{
	...
	int len = ImFormatStringV(nullptr, 0, fmt, args);
								...
								const ImRect* clip_rect)
{
	...
	else { draw_list->AddText(nullptr, 0.0f, pos, GetColorU32(ImGuiCol_Text), text, text_display_end, 0.0f, nullptr); }
}
	...
	: DrawListInst(&context->DrawListSharedData)
{
	...
	RootWindow = nullptr;
	RootWindowForTitleBarHighlight = nullptr;
	RootWindowForNav = nullptr;

	NavLastIds[0] = NavLastIds[1] = 0;
	NavRectRel[0] = NavRectRel[1] = ImRect();
	NavLastChildNavWindow = nullptr;
}
...
void ImGui::ClearActiveID() { SetActiveID(0, nullptr); }
...
bool ImGui::FocusableItemRegister(ImGuiWindow* window, ImGuiID id)
{
	...
	if (g.ActiveId == id && g.FocusTabPressed && !(g.ActiveIdBlockNavInputFlags & (1 << ImGuiNavInput_KeyTab_)) && g.
		FocusRequestNextWindow == nullptr)
	{
...
void ImGui::UpdateMouseMovingWindowEndFrame()
{
	...
	if (g.IO.MouseClicked[0])
	{
		...
		else if (g.NavWindow != nullptr && GetTopMostPopupModal() == nullptr)
		{
...
static void ImGui::UpdateMouseInputs()
{
	...
	for (int i = 0; i < IM_ARRAYSIZE(g.IO.MouseDown); i++)
	{
		...
		if (g.IO.MouseClicked[i])
		{
			...
			else { g.IO.MouseClickedTime[i] = g.Time; }
...
void ImGui::UpdateHoveredWindowAndCaptureFlags()
{
	...
	if (g.WantCaptureMouseNextFrame != -1)
		...
		g.IO.WantCaptureMouse = (mouse_avail_to_imgui && (g.HoveredWindow != nullptr || mouse_any_down)) || (!g
																											.
																											OpenPopupStack
																											.empty());
...
static void AddDrawListToDrawData(ImVector<ImDrawList*>* out_list, ImDrawList* draw_list)
{
	...
	IM_ASSERT(draw_list->VtxBuffer.Size == 0 || draw_list->_VtxWritePtr == draw_list->VtxBuffer.Data + draw_list->
VtxBuffer.Size);
	IM_ASSERT(draw_list->IdxBuffer.Size == 0 || draw_list->_IdxWritePtr == draw_list->IdxBuffer.Data + draw_list->
IdxBuffer.Size);
...
void ImGui::EndFrame()
{
	...
	if (g.IO.ImeSetInputScreenPosFn && (g.PlatformImeLastPos.x == FLT_MAX ||
...
void ImGui::Render()
{
	...
	windows_to_render_top_most[0] = (g.NavWindowingTarget && !(g.NavWindowingTarget->Flags &
										...
										: nullptr;
	windows_to_render_top_most[1] = g.NavWindowingTarget ? g.NavWindowingList : nullptr;
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
	if (g.DrawData.CmdListsCount > 0 && g.IO.RenderDrawListsFn != nullptr)
...
static ImVec2 CalcSizeAutoFit(ImGuiWindow* window, const ImVec2& size_contents)
{
	...
	bool will_have_scrollbar_x = (size_auto_fit_after_constraint.x - size_pad.x - size_decorations.x < size_contents.x
	...
	bool will_have_scrollbar_y = (size_auto_fit_after_constraint.y - size_pad.y - size_decorations.y < size_contents.y
		&& !(window->Flags & ImGuiWindowFlags_NoScrollbar)) || (window->Flags & ImGuiWindowFlags_AlwaysVerticalScrollbar
	);
...
static ImVec2 CalcNextScrollFromScrollTargetAndClamp(ImGuiWindow* window, bool snap_on_edges)
{
	...
	if (window->ScrollTarget.x < FLT_MAX)
	{
		...
		if (snap_on_edges && cr_x <= 0.0f && target_x <= window->WindowPadding.x)
			target_x = 0.0f;
		else if (snap_on_edges && cr_x >= 1.0f && target_x >= window->ContentSize.x + window->WindowPadding.x + GImGui
																												->Style.
																												ItemSpacing
																												.x)
										...
										bool* p_open)
{
	...
	if (flags & ImGuiWindowFlags_UnsavedDocument)
	{
		...
		RenderTextClipped(marker_pos + off, layout_r.Max + off, UNSAVED_DOCUMENT_MARKER, nullptr, nullptr,
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
	...
	ImGuiWindow* parent_window_in_stack = g.CurrentWindowStack.empty() ? nullptr : g.CurrentWindowStack.back();
	...
	if (g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasPos)
	{
		...
		else { SetWindowPos(window, g.NextWindowData.PosVal, g.NextWindowData.PosCond); }
	}
	...
	if (first_begin_of_the_frame)
	{
		...
		if (g.NavWindowingList != nullptr && (window->Flags & ImGuiWindowFlags_NoNavFocus) == 0)
		...
		window->ContentsRegionRect.Max.x = window->ContentsRegionRect.Min.x + (window->ContentSizeExplicit.x != 0.0f
																					? window->ContentSizeExplicit.x
																					: (window->Size.x - window
																										->WindowPadding.
																										x * 2.0f -
																						window->ScrollbarSizes.x));
		window->ContentsRegionRect.Max.y = window->ContentsRegionRect.Min.y + (window->ContentSizeExplicit.y != 0.0f
																					? window->ContentSizeExplicit.y
...
void ImGui::SetScrollHereX(float center_x_ratio)
{
	...
	target_x += (last_item_width * center_x_ratio) + (GImGui->Style.ItemSpacing.x * (center_x_ratio - 0.5f) * 2.0f);
...
void ImGui::SetScrollHereY(float center_y_ratio)
{
	...
	target_y += (window->DC.PrevLineSize.y * center_y_ratio) + (GImGui->Style.ItemSpacing.y * (center_y_ratio - 0.5f) *
...
void ImGui::OpenPopupEx(ImGuiID id)
{
	...
	else
	{
		...
		if (g.OpenPopupStack[current_stack_size].PopupId == id && g.OpenPopupStack[current_stack_size].OpenFrameCount ==
			g.FrameCount - 1) { g.OpenPopupStack[current_stack_size].OpenFrameCount = popup_ref.OpenFrameCount; }
		else
		{
...
void ImGui::CloseCurrentPopup()
{
	...
	while (popup_idx > 0)
	{
		...
		if (popup_window && (popup_window->Flags & ImGuiWindowFlags_ChildMenu))
			if (parent_popup_window == nullptr || !(parent_popup_window->Flags & ImGuiWindowFlags_Modal))
				close_parent = true;
		if (!close_parent)
			break;
		popup_idx--;
...
bool ImGui::BeginPopupEx(ImGuiID id, ImGuiWindowFlags extra_flags)
{
	...
	bool is_open = Begin(name, nullptr, extra_flags | ImGuiWindowFlags_Popup);
...
{
	ImGuiContext& g = *GImGui;
	g.NavAnyRequest = g.NavMoveRequest || g.NavInitRequest || (IMGUI_DEBUG_NAV_SCORING && g.NavWindow != nullptr);
	if (g.NavAnyRequest)
...
void ImGui::NavInitWindow(ImGuiWindow* window, bool force_reinit)
{
	...
	else { g.NavId = window->NavLastIds[0]; }
}
...
#undef NAV_MAP_KEY
	...
	if (g.NavWindow && g.NavWindow->NavLastChildNavWindow != nullptr && g.NavLayer == 0)
	...
	g.IO.NavVisible = (g.IO.NavActive && g.NavId != 0 && !g.NavDisableHighlight) || (g.NavWindowingTarget != nullptr);
	...
	if (g.NavMoveRequestForward == ImGuiNavForward_None)
	{
			...
			ImGuiWindowFlags_NoNavInputs))
		{
			if ((allowed_dir_flags & (1 << ImGuiDir_Left)) && IsNavInputPressedAnyOfTwo(
				ImGuiNavInput_DpadLeft, ImGuiNavInput_KeyLeft_, ImGuiInputReadMode_Repeat)) g.NavMoveDir =
				ImGuiDir_Left;
			if ((allowed_dir_flags & (1 << ImGuiDir_Right)) && IsNavInputPressedAnyOfTwo(
				ImGuiNavInput_DpadRight, ImGuiNavInput_KeyRight_, ImGuiInputReadMode_Repeat)) g.NavMoveDir =
				ImGuiDir_Right;
			...
			if ((allowed_dir_flags & (1 << ImGuiDir_Down)) && IsNavInputPressedAnyOfTwo(
				ImGuiNavInput_DpadDown, ImGuiNavInput_KeyDown_, ImGuiInputReadMode_Repeat)) g.NavMoveDir =
				ImGuiDir_Down;
		}
...
static float ImGui::NavUpdatePageUpPageDown(int allowed_dir_flags)
{
	...
	if (g.NavMoveDir != ImGuiDir_None || g.NavWindow == nullptr)
		return 0.0f;
	if ((g.NavWindow->Flags & ImGuiWindowFlags_NoNavInputs) || g.NavWindowingTarget != nullptr || g.NavLayer != 0)
		return 0.0f;
...
static void ImGui::NavUpdateWindowing()
{
	...
	if ((g.ActiveId == 0 || g.ActiveIdAllowOverlap) && g.NavWindowingToggleLayer && IsNavInputPressed(
...
void ImGui::NavUpdateWindowingList()
{
	...
	Begin("###NavWindowingList", nullptr,
...
bool ImGui::BeginDragDropSource(ImGuiDragDropFlags flags)
{
	...
	if (!(flags & ImGuiDragDropFlags_SourceExtern))
	{
		...
		else { g.ActiveIdAllowOverlap = false; }
		if (g.ActiveId != source_id)
			return false;
...
bool ImGui::BeginDragDropTargetCustom(const ImRect& bb, ImGuiID id)
{
	...
	ImGuiWindow* window = g.CurrentWindow;
	if (g.HoveredWindow == nullptr || window->RootWindow != g.HoveredWindow->RootWindow)
		return false;
	IM_ASSERT(id != 0);
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
	...
	struct Funcs
	{
		static ImRect GetWindowRect(ImGuiWindow* window, int rect_type)
		{
			...
			if (rect_type == WRT_OuterRectClipped) { return window->OuterRectClipped; }
			if (rect_type == WRT_InnerRect) { return window->InnerRect; }
			if (rect_type == WRT_InnerClipRect) { return window->InnerClipRect; }
			if (rect_type == WRT_WorkRect) { return window->WorkRect; }
			...
			if (rect_type == WRT_ContentsRegionRect) { return window->ContentsRegionRect; }
			IM_ASSERT(0);
			return ImRect();
		}
		...
		static void NodeDrawList(ImGuiWindow* window, ImDrawList* draw_list, const char* label)
		{
				...
				pcmd->ElemCount, pcmd++)
			{
				...
				Text("ElemCount: %d, ElemCount/3: %d, VtxOffset: +%d, IdxOffset: +%d", pcmd->ElemCount,
		...
		static void NodeWindow(ImGuiWindow* window, const char* label)
		{
			...
			if (!TreeNode(window, "%s '%s', %d @ 0x%p", label, window->Name, (window->Active || window->WasActive),
						window))
				return;
			ImGuiWindowFlags flags = window->Flags;
			...
			BulletText("Scroll: (%.2f/%.2f,%.2f/%.2f)", window->Scroll.x, window->ScrollMax.x, window->Scroll.y,
						window->ScrollMax.y);
			...
			BulletText("Appearing: %d, Hidden: %d (CanSkip %d Cannot %d), SkipItems: %d", window->Appearing,
						...
						window->SkipItems);
			BulletText("NavLastIds: 0x%08X,0x%08X, NavLayerActiveMask: %X", window->NavLastIds[0],
			...
			if (!window->NavRectRel[0].IsInverted())
				BulletText("NavRectRel[0]: (%.1f,%.1f)(%.1f,%.1f)", window->NavRectRel[0].Min.x,
							window->NavRectRel[0].Min.y, window->NavRectRel[0].Max.x, window->NavRectRel[0].Max.y);
			else
				BulletText("NavRectRel[0]: <None>");
			if (window->RootWindow != window) NodeWindow(window->RootWindow, "RootWindow");
			if (window->ParentWindow != nullptr) NodeWindow(window->ParentWindow, "ParentWindow");
		...
		static void NodeTabBar(ImGuiTabBar* tab_bar)
		{
			...
			ImFormatString(p, buf_end - p, "TabBar (%d tabs)%s", tab_bar->Tabs.Size,
							(tab_bar->PrevFrameVisible < GetFrameCount() - 2) ? " *Inactive*" : "");
			if (TreeNode(tab_bar, "%s", buf))
			{
				for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
				{
					...
					PushID(tab);
					if (SmallButton("<")) { TabBarQueueChangeTabOrder(tab_bar, tab, -1); }
					SameLine(0, 2);
					if (SmallButton(">")) { TabBarQueueChangeTabOrder(tab_bar, tab, +1); }
					SameLine();
	...
	if (TreeNode("DrawList", "Active DrawLists (%d)", g.DrawDataBuilder.Layers[0].Size))
	{
		for (int i = 0; i < g.DrawDataBuilder.Layers[0].Size; i++)
			Funcs::NodeDrawList(nullptr, g.DrawDataBuilder.Layers[0][i], "DrawList");
		TreePop();
	}
	...
	if (TreeNode("Internal state"))
	{
		...
		IM_ASSERT(
IM_ARRAYSIZE(input_source_names) == ImGuiInputSource_COUNT);
	...
	if (TreeNode("Tools"))
	{
		...
		show_windows_rects |= Combo("##show_windows_rect_type", &show_windows_rect_type, wrt_rects_names,
									WRT_Count);
